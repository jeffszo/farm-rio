...
export async function getPendingCSCValidations(
  ...
  const statuses = [
    ...
    "review requested by the compliance team - customer",
...
export async function getPendingTaxValidations(page = 1, itemsPerPage = 10) {
  ...
  const { data, error, count } = await supabaseServerClient
    .from("customer_forms")
    .select("*", { count: "exact" })
    .in("status", ["approved by the compliance team", "review requested by the tax team - customer"]) // From CSC initial or rejected by tax
    .range(from, to)
    .order("created_at", { ascending: true })
...
export async function getPendingWholesaleValidations(page = 1, itemsPerPage = 10) {
  ...
  const { data, error, count } = await supabaseServerClient
    .from("customer_forms")
    .select("*", { count: "exact" })
    .in("status", ["pending", "review requested by the wholesale team - customer", "finished"]) // From Tax or rejected by wholesale
    .range(from, to)
    .order("created_at", { ascending: true })
